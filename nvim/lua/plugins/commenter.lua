return {
  {
    -- Built-in neovim commenting doesn't (yet) support toggling, so use this.
    ...
    config = function()
      ...
      -- Remove the vim/_defaults.lua binding, which clashes as a prefix and makes it
      -- awkward to comment a single line.
      vim.keymap.del({ 'n' }, 'gcc')

      -- Line-wise invert comments with gc, in normal and visual mode
      vim.keymap.set({ 'x' }, 'gc', '<cmd>set operatorfunc=v:lua.__flip_flop_comment<cr>g@', { silent = true, desc = 'Invert [c]omments' })
      vim.keymap.set({ 'n' }, 'gc', 'V<cmd>set operatorfunc=v:lua.__flip_flop_comment<cr>g@', { silent = true, desc = 'Invert [c]omments' })

      --- operatorfunc to invert comments on each line
